...
@Controller
public class EmedicalRecordController{
	...
	@Autowired
	private EmedicalRecordService emedicalRecordService; 
	...
	public TMsgResponse<MedialRecordUploadResp> upload(@RequestBody EmedicalRecord emedicalRecord , Locale locale)
	{
		...
		{
			//病例上传
			msg.setResult(emedicalRecordService.insertEmedicalRecord(emedicalRecord));
		}catch(Exception ex)
		{
			ex.printStackTrace();
			//设置错误代码及提示消息
	...
	public TMsgResponse<Object> forwardMedicalRecord2Patient(Locale locale, @RequestBody MedialRecordForwardReq medialRecordForwardReq)
	{
		...
		{
			//病例上传
			msg.setResult(emedicalRecordService.forwardMedicalRecord2Patient(medialRecordForwardReq));
		}catch(Exception ex)
		{
			ex.printStackTrace();
			//设置错误代码及提示消息
	...
	public TMsgResponse<EmedicalRecord> detail(String emrId , Locale locale)
	{
		...
		{
			//查询
			msg.result = emedicalRecordService.queryEmedicalRecordById(emrId);
	...
	public TMsgResponse<String> delete(String emrId , Locale locale)
	{
		...
		{
			//删除
			emedicalRecordService.deleteEmedicalRecord(emrId);
	...
	public TMsgResponse<Page<EmedicalRecord>> medicalRecordBookList(Long userId , Integer role , Long patientId , Integer page , Integer num , Locale locale)
	{
		...
		{
			//删除
			msg.result = emedicalRecordService.selectEmedicalRecordList(userId , role , patientId, page, num);
	...
	public TMsgResponse<String> modify(@RequestBody EmedicalRecord emedicalRecord , Locale locale)
	{
		...
		{
			//修改电子病例
			emedicalRecordService.updateEmedicalRecord(emedicalRecord);
