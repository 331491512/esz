...
import com.esuizhen.cloudservice.statistics.constant.ExcelTemplate;
import com.esuizhen.cloudservice.statistics.service.AbstractExportBigData;
		...
		for(int i=0;i<len;i++) {
			...
			if(map.get("subTitles") != null) {
				...
				for(int j=0;j < length;j++) {
					...
					subTitles.append("<td class=\"txt\" colspan='"+cols+"' rowspan='"+rows+"'>" + HtmlUtils.htmlEscape(title == null ? "": title.toString()) + "</td>");
				}
			}
		}
		if (subTitles.length()>0) {
			writeToOutputStream("<tr>", fos);
			writeToOutputStream(subTitles.toString(), fos);
			writeToOutputStream("</tr>", fos);
		}
	}
