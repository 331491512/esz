...
import com.esuizhen.client.sync.dao.TreatmentNoteDao;
...
public class EciTreatmentNoteServiceImpl implements DataSyncService {
	...
	public int pushDataToServer(TBatchDetailInfo detailInfo) {
		int totalnum = 0;
		try {
			List<LinkedHashMap> l_treatmentNote = treatmentNoteDao.getBatchSyncTreatmentNote(detailInfo);
			totalnum = l_treatmentNote.size();

			if (l_treatmentNote.size() > 0) {
				// -->开始推送治疗信息
				// 01封装治疗信息
				String treatmentNote_json_data = PushDataTransfer.transfer(detailInfo.getBatchId(), 205,
						l_treatmentNote);

				// 02推送治疗信息
				String res1 = ClientRequestServerUtil.pushBatchData(treatmentNote_json_data);
				...
				if (null != msg && msg.getRespCode() == 0) {
					...
					for (LinkedHashMap d : l_treatmentNote) {
						treatmentIds = treatmentIds +"'"+ d.get("treatmentId").toString().replace(" ","") + "',";
					}
					
					treatmentIds = treatmentIds.substring(0,treatmentIds.length()-1);
					
					treatmentNoteDao.updateTreatmentNoteSyncflag(treatmentIds);
				} else {
					totalnum = -1;
				}
			}
	...
	public int getResultFromServer(TBatchDetailInfo detailInfo) {
		...
		try {
			...
			if(null!=msg && msg.getRespCode()==0){
				...
				if(l_tBatchDataResultInfo.size()>0){
					for (Object obj : l_tBatchDataResultInfo) {
						...
						treatmentNoteDao.updateSyncResult(result);
			...
			LogUtil.logError.error("<ERROR>---治疗信息同步结果获取失败==>>>:"+e.getMessage());
			e.printStackTrace();
			totalnum=-1;
		}
		return totalnum;
