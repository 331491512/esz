...
import com.esuizhen.client.sync.dao.RDoctorPatientDao;
...
public class RDoctorPatientServiceImpl implements DataSyncService {
	...
	@Autowired
	private RDoctorPatientDao rDoctorPatientDao;
	...
	public int pushDataToServer(TBatchDetailInfo detailInfo) {
		int totalnum = 0;
		try {
			List<LinkedHashMap> l_rDoctorPatient = rDoctorPatientDao.getBatchSyncRDoctorPatient(detailInfo);
			totalnum = l_rDoctorPatient.size();

			if (l_rDoctorPatient.size() > 0) {
				// -->开始推送医患关系信息
				// 01封装门诊信息
				String rDoctorPatient_json_data = PushDataTransfer.transfer(detailInfo.getBatchId(), 105,
						l_rDoctorPatient);

				// 02推送医患关系信息
				String res1 = ClientRequestServerUtil.pushBatchData(rDoctorPatient_json_data);
				...
				if (null != msg && msg.getRespCode() == 0) {
					...
					for (LinkedHashMap d : l_rDoctorPatient) {
						ids = ids + d.get("id") + ",";
					}
					
					ids = ids.substring(0,ids.length()-1);
					
					rDoctorPatientDao.updateRDoctorPatientSyncflag(ids);
				} else {
					totalnum = -1;
				}
			}
	...
	public int getResultFromServer(TBatchDetailInfo detailInfo) {
		...
		try {
			...
			if(null!=msg && msg.getRespCode()==0){
				...
				if(l_tBatchDataResultInfo.size()>0){
					for (Object obj : l_tBatchDataResultInfo) {
						...
						rDoctorPatientDao.updateSyncResult(result);
			...
			LogUtil.logError.error("<ERROR>---医患关系信息同步结果获取失败==>>>:"+e.getMessage());
			e.printStackTrace();
			totalnum=-1;
		}
		return totalnum;
