...
import com.esuizhen.client.sync.dao.RPatientUuidDao;
...
public class RUuidPatientNoServiceImpl implements DataSyncService {
	...
	@Autowired
	private RPatientUuidDao rPatientUuidDao;
	...
	public int pushDataToServer(TBatchDetailInfo detailInfo) {
		int totalnum = 0;
		try {
			List<LinkedHashMap> l_rPatientUuid = rPatientUuidDao.getBatchSyncRPatientUuid(detailInfo);
			totalnum = l_rPatientUuid.size();

			if (l_rPatientUuid.size() > 0) {
				// -->开始推送rPatientUuid信息
				// 01封装信息
				String rPatientUuid_json_data = PushDataTransfer.transfer(detailInfo.getBatchId(), 104, l_rPatientUuid);

				// 02推送rPatientUuid信息
				String res1 = ClientRequestServerUtil.pushBatchData(rPatientUuid_json_data);
				...
				if (null != msg && msg.getRespCode() == 0) {
					...
					for (LinkedHashMap d : l_rPatientUuid) {
						ids = ids + d.get("id") + ",";
					}
					
					ids = ids.substring(0,ids.length()-1);
					
					rPatientUuidDao.updateRPatientUuidSyncflag(ids);
				} else {
					totalnum = -1;
				}
			}
	...
	public int getResultFromServer(TBatchDetailInfo detailInfo) {
		...
		try {
			...
			if(null!=msg && msg.getRespCode()==0){
				...
				if(l_tBatchDataResultInfo.size()>0){
					for (Object obj : l_tBatchDataResultInfo) {
						...
						rPatientUuidDao.updateSyncResult(result);
			...
			LogUtil.logError.error("<ERROR>---RPATIENTUUID信息同步结果获取失败==>>>:"+e.getMessage());
			e.printStackTrace();
			totalnum=-1;
		}
		return totalnum;
