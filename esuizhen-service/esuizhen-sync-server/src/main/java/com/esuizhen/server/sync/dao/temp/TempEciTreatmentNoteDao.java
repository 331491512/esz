package com.esuizhen.server.sync.dao.temp;

import java.util.List;

import com.esuizhen.server.sync.bean.TBatchDetailInfo;
import com.esuizhen.server.sync.bean.server.TreatmentNoteRes;
import com.esuizhen.server.sync.model.temp.SyncEciTreatmentNote;

public interface TempEciTreatmentNoteDao {

	void insert(SyncEciTreatmentNote eciTreatmentNote);

	List<TreatmentNoteRes> getSyncTreatmentNote(TBatchDetailInfo detail);
}
