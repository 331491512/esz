...
@Controller
public class FaultPatientController {
	...
	@ResponseBody
	@RequestMapping(value="/fault/patient/list",method=RequestMethod.POST)
	public TMsgResponse<Map<String,Object>> queryFaultPatient(@RequestBody TPatientSearchInfo patientSearch,Locale locale) {
		...
		patientSearch.setFaultType(99);
		try {
			patientSearch.setInterfaceName("/user/fault/patient/list");
	...
	@ResponseBody
	@RequestMapping(value="/fault/patient/statistics",method=RequestMethod.GET)
	public TMsgResponse<TFaultSimilarPatientSpread> statisticsFaultPatient(Integer searchId,Locale locale) {
		TMsgResponse<TFaultSimilarPatientSpread> tMsgResponse = new TMsgResponse<TFaultSimilarPatientSpread>();
		tMsgResponse.setRespCode(ErrorMessage.SUCCESS.getCode());
		tMsgResponse.setRespMsg(messageSource.getMessage(ErrorMessage.SUCCESS.info, null, locale));
		try {
			tMsgResponse.result = patientFollowupService.statisticsFaultPatient(searchId);
	...
	@ResponseBody
	@RequestMapping(value="/fault/patient/diseasetype/statistics",method=RequestMethod.POST)
	public TMsgResponse<Integer> statisticsDiseaseTypePatient(@RequestBody TPatientSearchInfo  patientSearchInfo,Locale locale) {
		TMsgResponse<Integer> tMsgResponse = new TMsgResponse<Integer>();
		tMsgResponse.setRespCode(ErrorMessage.SUCCESS.getCode());
		tMsgResponse.setRespMsg(messageSource.getMessage(ErrorMessage.SUCCESS.info, null, locale));
		try {
			tMsgResponse.result = patientFollowupService.statisticsPatientByDiseaseType(patientSearchInfo);
		} catch (Exception ex) {
			LogUtil.logError.error(ex.getCause() + "\t" + ex.getMessage());
			tMsgResponse.setRespCode(ErrorMessage.E1500.getCode());
			tMsgResponse.setRespMsg(messageSource.getMessage(ErrorMessage.E1500.info, null, locale));
